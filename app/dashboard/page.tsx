...
import { Zap, Shield, Eye, FileText, Link2, Target } from "lucide-react";
import { useIsMobile } from "../lib/useIsMobile";
import {
  FullPageShell, PageHeader, StatCard, SectionLabel,
    ...
    <FullPageShell type="system">
      ...
      {loading ? (
        ...
        <>
            ...
            {STAT_CARDS.map((stat, i) => {
              ...
              return (
                <StatCard
                  ...
                  label={stat.label}
                  value={stat.value}
                  icon={<Icon size={12} strokeWidth={1.5} style={{ color: stat.color }} />}
                />
              );
            })}
          </div>
          ...
          <div style={{
            display: "grid",
            gridTemplateColumns: accuracy ? (isMobile ? "1fr" : "200px 1fr") : "1fr",
            ...
            {accuracy && (
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 8 }}>
                  ...
                  <span style={{ fontSize: 10, color: Z600 }}>Prediction Accuracy</span>
                </div>
                <div style={{
                ...
                <div style={{ fontSize: 10, color: Z600, marginTop: 4 }}>
            ...
            {patterns && patterns.patterns && patterns.patterns.length > 0 && (
              <div>
                <SectionLabel style={{ marginBottom: 10 }}>Patterns</SectionLabel>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {patterns.patterns.map((p: any, i: number) => {
                    const tc: Record<string, { color: string; bg: string }> = {
                      strength: { color: "#22c55e", bg: "rgba(34,197,94,0.06)" },
                      weakness: { color: "#f59e0b", bg: "rgba(245,158,11,0.06)" },
                      blind_spot: { color: "#ef4444", bg: "rgba(239,68,68,0.06)" },
                    };
                    ...
                    return (
                      <div key={i} style={{
                        ...
                        <div style={{ fontSize: 12, color: Z200, fontWeight: 500 }}>{p.insight}</div>
                        <div style={{ fontSize: 10, color: Z600, marginTop: 2 }}>{p.evidence}</div>
                      </div>
                    );
                  })}
                </div>
                {patterns.recommendation && (
                  <div style={{
                    ...
                    background: "rgba(255,255,255,0.02)", border: `1px solid ${Z800}`,
                ...
                <ListItem key={item.id}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{
                        fontSize: 13, fontWeight: 500, color: Z200,
                        overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                      }}>
                        {item.summary?.slice(0, 100) || item.context_type}
                      </div>
                      <div style={{ fontSize: 11, color: Z600 }}>
