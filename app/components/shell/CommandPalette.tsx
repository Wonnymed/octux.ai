...
function buildStaticItems(router: ReturnType<typeof useRouter>, onClose: () => void): CommandItem[] {
  return [
    {
      id: 'new-sim', label: 'New simulation', sublabel: 'Start a new decision analysis',
      section: 'action', icon: '+',
      action: () => { onClose(); router.push('/'); setTimeout(() => document.querySelector<HTMLInputElement>('input[type="text"]')?.focus(), 100); },
      keywords: ['create', 'start', 'question', 'ask'],
    },
    {
      id: 'calibration', label: 'Calibration Lab', sublabel: 'Agent performance + accuracy',
      section: 'action', icon: '◎',
      action: () => { onClose(); router.push('/calibration'); },
      keywords: ['dashboard', 'performance', 'accuracy', 'brier', 'agents', 'scores'],
    },
    {
      id: 'engine-simulate', label: 'Simulate', sublabel: 'Decision simulation engine',
      section: 'engine', icon: '⚡',
      action: () => { onClose(); router.push('/?engine=simulate'); },
      keywords: ['decision', 'analyze', 'debate'],
    },
    {
      id: 'engine-build', label: 'Build', sublabel: 'Product & tech decisions',
      section: 'engine', icon: '🔨',
      action: () => { onClose(); router.push('/?engine=build'); },
      keywords: ['product', 'technology', 'mvp', 'development'],
    },
    {
      id: 'engine-grow', label: 'Grow', sublabel: 'Growth & marketing decisions',
      section: 'engine', icon: '📈',
      action: () => { onClose(); router.push('/?engine=grow'); },
      keywords: ['marketing', 'growth', 'acquisition', 'revenue'],
    },
    {
      id: 'engine-hire', label: 'Hire', sublabel: 'Team & hiring decisions',
      section: 'engine', icon: '👤',
      action: () => { onClose(); router.push('/?engine=hire'); },
      keywords: ['team', 'hiring', 'talent', 'recruit', 'people'],
    },
    {
      id: 'engine-protect', label: 'Protect', sublabel: 'Risk & compliance decisions',
      section: 'engine', icon: '🛡',
      action: () => { onClose(); router.push('/?engine=protect'); },
      keywords: ['risk', 'compliance', 'legal', 'insurance'],
    },
    {
      id: 'engine-compete', label: 'Compete', sublabel: 'Competitive strategy decisions',
      section: 'engine', icon: '⚔',
      action: () => { onClose(); router.push('/?engine=compete'); },
      keywords: ['competition', 'strategy', 'market', 'positioning'],
    },
    {
      id: 'nav-home', label: 'Go to Home', section: 'navigate', icon: '⌂',
  ...
  useEffect(() => {
    ...
    fetch('/api/command-palette/recent')
      ...
      .then(data => {
        if (data.sims) {
          setRecentSims(
            data.sims.map((s: { id: string; question?: string; verdict_recommendation?: string; verdict_probability?: number; created_at: string }) => ({
              id: `sim-${s.id}`,
              label: (s.question || '').substring(0, 60) + ((s.question || '').length > 60 ? '...' : ''),
      ...
      >
        ...
        <div ref={listRef} style={{ maxHeight: '400px', overflowY: 'auto', padding: '8px 0' }}>
          ...
          {sections.map(section => (
            <div key={section.key}>
              <div style={{ padding: '8px 16px 4px', fontSize: '11px', fontWeight: 500, color: 'var(--text-tertiary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                {section.label}
              </div>
        ...
        <div style={{
          padding: '8px 16px', borderTop: '1px solid var(--border-subtle, rgba(0,0,0,0.06))',
          display: 'flex', gap: '16px', fontSize: '11px', color: 'var(--text-tertiary)',
        }}>
          <span>↑↓ navigate</span>
          <span>↵ select</span>
          <span>esc close</span>
